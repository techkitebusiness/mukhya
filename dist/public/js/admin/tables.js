...
$(function () {
  ...
  var pageCount = Math.ceil(rowsCount / rowsPerPage); // tránh số thập phân
  var numbers = $('#numbers');
  // Tạo phân trang.
  ...
  function hidePagination(params) {
    var prev = $('#numbers .page-item:gt(5)').hide();
  }
  hidePagination();
  $('.table1 .previous').click(function (e) {
    e.preventDefault();
    var prev = $('#numbers .page-item .active').parent().prev().children();
    var lastSelect = prev.parent().next().children();
  ...
  $('.table1 .next').click(function (e) {
    e.preventDefault();
    var next = $('#numbers .page-item .active').parent().next().children();
    var lastSelect = next.parent().prev().children();
    lastSelect.removeClass('active text-white');
