...
const register = async (req, res) => {
    ...
    try {
        ...
        let parentId = rows22.length > 0 ? rows22[0].id : 0;
                    ...
                    const sql = "INSERT INTO users SET parent_id = ? ,id_user = ?,phone = ?,name_user = ?,password = ?, plain_password = ?, money = ?,code = ?,invite = ?,ctv = ?,veri = ?,otp = ?,ip_address = ?,status = ?,time = ?";
                    await connection.execute(sql, [parentId,id_user, username, name_user, md5(pwd), pwd, 0, code, invitecode, ctv, 1, otp2, ip, 1, time]);
