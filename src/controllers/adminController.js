...
const listDailySalaryIncome = async (req, res) => {
    ...
    const [total_users] = await connection.query(`SELECT * FROM commission WHERE 1`);
    return res.status(200).json({
        message: 'Success',
        status: true,
        datas: users,
...
if (type == 'confirm') {
    ...
    await connection.query('UPDATE users SET temp_money = temp_money + ? , money = money + ?, total_money = total_money + ? WHERE phone = ?', [amountAdd,amountAdd, amountAdd, info[0].phone]);

    const [rows2] = await connection.query(`SELECT * FROM users WHERE phone = ?`, [info[0].phone]);
    ...
    if (parentPhone != '') {
        ...
        if (process.length > 0) {
            ...
            let amountSponsor =   (info[0].money / 100) * 4;
            const sql = "INSERT INTO inc_direct SET process_id = ?, phone = ?, from_id = ?, total_amount = ?, returns = ?, net_amount = ?, date = ?";
            await connection.execute(sql, [Pid, parentPhone, info[0].phone, info[0].money, 4, amountSponsor, Pdate]);
